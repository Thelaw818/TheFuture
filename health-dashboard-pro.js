(function(){
  ...
  function getMetrics(){
    ...
    const weight=latest(health,'weight')||latest(health,'weightLb')||latest(health,'weightlbs')||192.4;
    const systolic=latest(bp,'sys')||118, diastolic=latest(bp,'dia')||76, pulse=latest(bp,'pulse')||latest(health,'pulse')||68;
    const sleepHours=latest(sleep,'hours')||7.8, mood=latest(sleep,'mood')||8, energy=latest(sleep,'energy')||8;
    const healthScore=Math.max(0,Math.min(100,Math.round((sleepHours/8)*35+(mood/10)*25+(energy/10)*20+20)));
    const waterTarget=94, water=Number(localStorage.getItem('hd_today_water_oz')||64); const steps=Number(localStorage.getItem('hd_today_steps')||6523);
    return {weight,systolic,diastolic,pulse,sleepHours,mood,energy,healthScore,waterTarget,water,steps,meds};
  }
  function getSeries(){const health=store('health').slice(-7), bp=store('hd_bp_readings').slice(-7); let weights=health.map(r=>n(r.weight)||n(r.weightLb)||n(r.weightlbs)).filter(v=>v!==null); if(!weights.length)weights=[193.6,193.9,194.2,194.1,194.1,194.6,195.0]; let sys=bp.map(r=>n(r.sys)).filter(v=>v!==null); if(!sys.length)sys=[100,99,99,103,102,104,102]; let hr=bp.map(r=>n(r.pulse)).filter(v=>v!==null); if(!hr.length)hr=[66,66,66,66,65,67,66]; return {weights,sys,hr};}
  function dashboardHTML(){const m=getMetrics(), s=getSeries(); const tasks=JSON.parse(localStorage.getItem('hd_dashboard_tasks')||'{}'); const taskList=['Log Weight','Medication','Drink 40 oz of Water','Evening Journal','10 min Walk / Movement'];
    ...
    <div class="hd-metric-grid"><div class="hd-metric"><div class="hd-icon">💙</div><h3>Health Score</h3><div class="hd-value">${m.healthScore}<small>/100</small></div><div class="hd-sub">Live from sleep, mood, and energy</div><div class="hd-bar"><span style="width:${m.healthScore}%"></span></div></div><div class="hd-metric sleep"><div class="hd-icon">🌙</div><h3>Sleep</h3><div class="hd-value">${m.sleepHours.toFixed(1)}h</div><div class="hd-sub">Latest sleep log</div><div class="hd-bar"><span style="width:${Math.min(100,m.sleepHours/8*100)}%"></span></div></div><div class="hd-metric water"><div class="hd-icon">💧</div><h3>Water</h3><div class="hd-value">${Math.round(m.water/m.waterTarget*100)}%</div><div class="hd-sub">${m.water} oz / ${m.waterTarget} oz</div><div class="hd-bar"><span style="width:${Math.min(100,m.water/m.waterTarget*100)}%"></span></div></div><div class="hd-metric steps"><div class="hd-icon">🚶</div><h3>Steps</h3><div class="hd-value">${m.steps.toLocaleString()}</div><div class="hd-sub">Goal: 10,000</div><div class="hd-bar"><span style="width:${Math.min(100,m.steps/10000*100)}%"></span></div></div><div class="hd-metric mood"><div class="hd-icon">😊</div><h3>Mood</h3><div class="hd-value">${m.mood>=8?'Good':m.mood>=5?'Okay':'Low'}</div><div class="hd-sub">${m.mood}/10</div><div class="hd-bar"><span style="width:${m.mood*10}%"></span></div></div></div>
    ...
    <div class="hd-lower-grid"><div class="hd-card"><h3>Weight Progress</h3><div class="hd-mini-stat">${m.weight}<small> lbs</small></div><p class="hd-sub">Goal tracking updates from Health Overview entries.</p><div class="hd-bar"><span style="width:76%"></span></div></div><div class="hd-card"><h3>Latest Vitals</h3><div class="hd-list-row"><span>Blood Pressure</span><strong>${m.systolic}/${m.diastolic} <small>mmHg</small> <em class="hd-status">Saved</em></strong></div><div class="hd-list-row"><span>Resting Heart Rate</span><strong>${m.pulse} <small>bpm</small></strong></div><button class="hd-open" data-hd-open="health">View All Vitals</button></div><div class="hd-card"><h3>Today's Journal</h3><p>${safe((store('journal').slice(-1)[0]||{}).notes || (store('journal').slice(-1)[0]||{}).entry || 'No journal entry yet. Open Journal to add notes for today.')}</p><button class="hd-open" data-hd-open="journal">Open Journal</button></div><div class="hd-card"><h3>Health Insights</h3><p>Your dashboard updates from local records saved in calculators, vitals, sleep/mood logs, medication, labs, and journal sections.</p></div><div class="hd-card"><h3>Upcoming Appointments</h3><div class="hd-list-row"><span>Next Check-in</span><strong>Not scheduled</strong></div><button class="hd-open" data-hd-open="journal">Add Note</button></div><div class="hd-card"><h3>Reminders</h3><div class="hd-list-row"><span>Medication</span><strong>${m.meds.length?safe(m.meds[0].time||'Today'):'Add first'}</strong></div><button class="hd-open" data-hd-open="medication">Open Medication</button></div></div></div>`;
  }
